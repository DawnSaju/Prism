...
const Starfield = ({
  ...
  const bigBang = () => {
    if (sd.current.star.arr.length !== quantity) {
      sd.current.star.arr = new Array(quantity).fill(undefined).map((): Star => [
  ...
  const mouseHandler = useCallback((event: MouseEvent) => {
    const el = canvasRef.current?.parentElement;
    if (el) {
  ...
  const tiltHandler = useCallback((event: DeviceOrientationEvent) => {
  ...
  }, [mouseAdjust, tiltAdjust, clickToWarp, mouseHandler, tiltHandler, clickHandler]);
